   ...
   <div class="col-xs-12 content pb-4">
      <div class="row">
       ...
       if(isset($_SESSION['login'])){
           include 'header.php';
            $id = $_POST['id'];
           $requete = "UPDATE `utilisateur` SET `nom_ut` = '', `prenom` = '', `email` = '',`telephone` = '', `adresse` = '', `numero` = '', `vilid` = '', `login` = '', password = '' WHERE `utilisateur`.`id` = 25";
            $bdd->exec($requete);
              ?>
            
            <p class="alert alert-info">Utilisateur supprimé.</p>
                    <?php
       }
       else {
